...
import { User as UserIcon, Calendar, Bell, LogOut, Shield, RefreshCw, Save } from 'lucide-react';
...
export default function Dashboard({ user, onLogout }: DashboardProps) {
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-slate-50 text-center">
        <div className="w-full max-w-sm bg-white p-8 rounded-2xl shadow-xl border border-slate-100">
          ...
          <p className="text-slate-500 mb-6">Aguardando aprovação do administrador.</p>
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50">
      ...
      {/* Top Navigation */}
      <div className="bg-white border-b border-slate-200 px-4 py-3 flex justify-between items-center sticky top-0 z-30">
        ...
        <div className="flex items-center gap-3">
          {isAdminOrDev && (
             <span className={`text-[10px] font-bold px-2 py-1 rounded-full uppercase ${isDev ? 'bg-indigo-900 text-white' : 'bg-purple-100 text-purple-700'}`}>
               {isDev ? 'DEV' : 'Admin'}
             </span>
          )}
              ...
              <div className="w-full mb-4">
                ...
                <div className="flex gap-2">
                    <input 
                        ...
                        className="flex-1 border border-slate-200 rounded-lg px-3 py-2 text-slate-800 text-sm focus:ring-2 focus:ring-blue-500 outline-none"
                    />
                    <button 
      ...
      <div className="bg-white border-t border-slate-200 flex justify-around p-2 pb-safe fixed bottom-0 left-0 right-0 z-40">
        <button 
          ...
          className={`flex flex-col items-center p-2 rounded-xl flex-1 ${activeTab === 'lists' ? 'text-blue-600' : 'text-slate-400'}`}
        >
          <Calendar size={24} />
        ...
        <button 
          ...
          className={`flex flex-col items-center p-2 rounded-xl flex-1 ${activeTab === 'profile' ? 'text-blue-600' : 'text-slate-400'}`}
        >
          <UserIcon size={24} />
        ...
        <button 
          ...
          className={`flex flex-col items-center p-2 rounded-xl flex-1 relative ${activeTab === 'notifications' ? 'text-blue-600' : 'text-slate-400'}`}
        >
          ...
          <span className="text-[10px] font-medium mt-1">Avisos</span>
        </button>
      </div>
    </div>
  );
